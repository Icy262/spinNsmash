...
class NetworkConnection:
	#TODO: add some mechanisim for preventing dos from spam
	...
	def recieve(self):
		...
		while True: #for the duration of the connection, try to read the next full packet and return it
			...
			NetworkObject_length = int.from_bytes(self.read_queue[:2], "big") #convert the first two bytes to an int. The int represents the length of the network object
			...
			message = self.read_queue[:NetworkObject_length] #pull just the full message out of the read_queue
			del self.read_queue[:NetworkObject_length] #remove the message we just read from the queue
			self.recieved_messages_queue.append(message) #add the message to the list of recieved messages that havn't been used yet
	
	def get_next_message(self):
		"""
		Will return the first message in the queue, or None if there are no pending messages
		"""
		return self.recieved_messages_queue.pop(0) if self.recieved_messages_queue else None #takes the first message out of the queue and returns it, if there is anything in the list, if not, returns None