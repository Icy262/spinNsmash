...
class PlayerContainer(abc.ABC):
	...
	"""
	def __init__(self):
		self.connected_players = [] #holds all the players being contained in this container
	...
	def transfer_player(self, player, other_container):
		...
		with threading.Lock(): #thread safety is fun
			if(player in self.connected_players): #removes the player from our list. list.remove() returns the value it removed. If this is None, it means the player is not in the list. We need to handle this because if not you will get None values getting added to other containers
				self.connected_players.remove(player)
				other_container.add_player(player) #adds the player to the other container
			else: #player wasn't in player list
				print("PLAYER NOT CONNECTED") #TODO: SHOULD PROBABLY HANDLE THIS PROPERLY LATER